<?php
  ...
  $user=$_SESSION['username'];
    ...
    $sql = $conn->query("SELECT starid FROM stars Where hotel='SixSeason'");
    $numR = $sql->num_rows;

    $sql = $conn->query("SELECT SUM(rateIndex) AS total FROM stars Where hotel='SixSeason'");
    $rData = $sql->fetch_array();
    $total = $rData['total'];

    $avg = round(($total / $numR),2);

    $conn->query("UPDATE hotels SET rating='$avg' WHERE hotelname='SixSeason'");
?>
<!doctype html>
<html lang="en">
<head>
