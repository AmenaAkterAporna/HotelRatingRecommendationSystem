<?php
  ...
  $user=$_SESSION['username'];
    ...
    $sql = $conn->query("SELECT starid FROM stars Where hotel='InterContinental'");
    $numR = $sql->num_rows;

    $sql = $conn->query("SELECT SUM(rateIndex) AS total FROM stars Where hotel='InterContinental'");
    $rData = $sql->fetch_array();
    $total = $rData['total'];

    $avg = round(($total / $numR),2);
    $conn->query("UPDATE hotels SET rating='$avg' WHERE hotelname='InterContinental'");
?>
<!doctype html>
<html lang="en">
<head>
...
<?php include 'Intercontinental.php';?>
</body>
</html>